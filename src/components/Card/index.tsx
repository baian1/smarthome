...
class Card extends React.PureComponent<P> {
  ...
  public render(): JSX.Element {
    const { name, deviceID, data } = this.props.device
    ...
    return (
      <div
        ...
        <div className={`${prefix}-left`}>
          <div className={`${prefix}-left-up ${contentCenter}`}>
            {name ? name : deviceID}
          </div>
          <div className={`${prefix}-left-down ${contentCenter}`}>
            ...
            <br />
            <div>
              温度:{data["Temperature and Humidity Sensor"].Temperature}
            </div>
          </div>
        </div>
        <div className={`${prefix}-right`}>
          <div className={`${prefix}-right-up ${contentCenter}`}>
            <div>
              震动:
              {data["Shock Sensor"]["on/off"] === "on"
                ? data["Shock Sensor"].status === "danger"
                  ? "危险"
                  : "正常"
                : "关闭"}
            </div>
            ...
            <div>
              红外:
              {data["Infrared Sensor"]["on/off"] === "on"
                ? data["Infrared Sensor"].status === "danger"
                  ? "危险"
                  : "正常"
                : "关闭"}
            </div>
          ...
          <div className={`${prefix}-right-down ${contentCenter}`}>
            <div>
              烟雾浓度:
              {data["Smoke Sensor"]["on/off"] === "on"
                ? data["Smoke Sensor"].concentration
                : "关闭"}
            </div>
            <br />
