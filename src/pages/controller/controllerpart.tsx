  ...
  const list = useMemo(() => {
    ...
    return data.map((value: Data, index: number) => {
      ...
      switch (mergemode) {
        case "button":
          return (
            <div key={key} className={`${prefix}-li`}>{value.title}
              <button className={`${prefix}-button`} disabled={value.disabled} onClick={handleOnClick(value.handle)}>{value.buttonContent}</button>
            </div>)
        case "switch":
          if (value === null) {
